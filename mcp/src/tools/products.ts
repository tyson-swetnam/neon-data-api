...
export function createProductTools(client: NeonApiClient): Tool[] {
  return [
    {
  ...
  try {
    switch (name) {
      ...
      case 'neon_search_products': {
        ...
        if (theme) {
          filteredProducts = filteredProducts.filter(product =>
            product.themes.some(t => t.toLowerCase().includes(theme.toLowerCase()))
          );
        }
        
        if (scienceTeam) {
          filteredProducts = filteredProducts.filter(product =>
            product.productScienceTeam.toLowerCase().includes(scienceTeam.toLowerCase())
          );
        }
