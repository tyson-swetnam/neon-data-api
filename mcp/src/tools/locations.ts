...
export function createLocationTools(client: NeonApiClient): Tool[] {
  return [
    {
      ...
      case 'neon_list_site_locations': {
        ...
        if (locationType) {
          filteredLocations = locations.filter(loc => 
            loc.locationType.toLowerCase().includes(locationType.toLowerCase())
          );
        }
      ...
      case 'neon_find_towers': {
        ...
        if (towerType) {
          ...
          towers = towers.filter(tower => 
            tower.locationDescription.toLowerCase().includes(searchTerm) ||
        ...
        if (latitude !== undefined && longitude !== undefined) {
          locations = locations.filter(loc => {
            const distance = calculateDistance(
              latitude, longitude,
              loc.locationDecimalLatitude, loc.locationDecimalLongitude
            );
