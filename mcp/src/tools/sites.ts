...
export function createSiteTools(client: NeonApiClient): Tool[] {
  return [
    {
...
export async function handleSiteTool(
  name: string, 
  args: any, 
    ...
    switch (name) {
      case 'neon_list_sites': {
        ...
        if (siteType) {
          sites = sites.filter(site => 
            site.siteType.toLowerCase().includes(siteType.toLowerCase())
          );
        }
      ...
      case 'neon_get_site_products': {
        ...
        const productsByLevel = site.dataProducts.reduce((acc, product) => {
          ...
          return acc;
        }, {} as Record<string, typeof site.dataProducts>);
